...
class WorksheetTemplate(models.Model):
    ...
    def _get_template_action_context(self):
        ...
        if self.res_model == 'project.task':
            context.update({
                'action_xml_id': 'custom_repair_product.fsm_worksheets_action_settings',
    ...
    def get_x_model_form_action(self):
        ...
        if self.res_model == 'project.task':
            action['context'].update({'action_xml_id': 'custom_repair_product.fsm_worksheets_action_settings', 'worksheet_template_id': self.id})
        return action
    ...
    def _create_demo_data_fsm(self, model_id=False):
        ...
        if not model_id:
            model_id = self.env.ref('custom_repair_product.fsm_worksheet_template2').model_id.id
