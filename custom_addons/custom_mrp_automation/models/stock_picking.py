...
class Stock_picking(models.Model):
    ...
    def _run_mrp_automation(self):
        ...
        record = self
        if record.picking_type_id.code == 'incoming' and record.state == 'done':
            # Debug: Crear nota en el chatter para verificar ejecución
            record.message_post(body="🔄 Iniciando automatización de órdenes de fabricación...")
            ...
            if not received_components:
                record.message_post(body="❌ No se encontraron componentes recibidos")
            else:
                ...
                record.message_post(body=debug_msg)
                ...
                record.message_post(body="🔍 BOMs encontradas que usan estos componentes: {}".format(len(matching_boms)))
                
                if not matching_boms:
                    record.message_post(body="⚠️ No se encontraron BOMs que utilicen los componentes recibidos")
                else:
                    orders_created = 0
                    
                    # Para cada BOM encontrada, verificar si podemos fabricar productos
                    for bom in matching_boms:
                        record.message_post(body="📋 Evaluando BOM: {}".format(bom.display_name))
                        ...
                        if not available_components:
                            record.message_post(body="⚠️ {}: No contiene ninguno de los componentes recibidos".format(bom.display_name))
                            continue
                        ...
                        if missing_components:
                            ...
                            if len(missing_components) > 3:
                                missing_text += '...'
                            record.message_post(body="ℹ️ {}: Componentes faltantes: {}".format(bom.display_name, missing_text))
                        ...
                        if len(available_list) > 3:
                            available_text += '...'
                        record.message_post(body="✅ {}: Componentes disponibles: {}".format(bom.display_name, available_text))
                        
                        # DECIDIR: ¿Crear órdenes solo si tenemos TODOS los componentes o permitir parciales?
                        # OPCIÓN 1: Requiere todos los componentes
                        if missing_components:
                            record.message_post(body="⚠️ {}: Saltando BOM - faltan componentes requeridos".format(bom.display_name))
                        ...
                        if max_units > 0 and max_units < 999999:
                            record.message_post(body="🏭 {}: Creando {} órdenes...".format(bom.display_name, max_units))
                            ...
                            for unit_num in range(max_units):
                                try:
                                    ...
                                    record.message_post(body="✅ Orden {} creada: {}".format(unit_num + 1, mo.name))
                                    
                                except Exception as e:
                                    error_msg = str(e)
                                    record.message_post(body="❌ Error creando orden {}: {}".format(unit_num + 1, error_msg))
                            
                            # Actualizar received_components después de procesar esta BOM
                            for k, v in temp_components.items():
                                received_components[k] = v
                        else:
                            record.message_post(body="⚠️ {}: No se pueden fabricar unidades completas (max_units: {})".format(bom.display_name, max_units))
                    
                    if orders_created > 0:
                        record.message_post(body="✅ Automatización completada: {} órdenes de fabricación creadas".format(orders_created))
                    else:
                        record.message_post(body="⚠️ No se pudieron crear órdenes de fabricación")
                        record.message_post(body="🔍 Posibles causas: BOMs requieren componentes no recibidos, cantidades insuficientes, o configuración de BOMs incorrecta")
        else:
            # Debug para condiciones no cumplidas
            record.message_post(body="ℹ️ Automatización no ejecutada - Tipo: {}, Estado: {}".format(record.picking_type_id.code, record.state))
